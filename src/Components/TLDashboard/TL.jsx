...
const TLDashboard = ({ setActiveContainer }) => {
  ...
  const renderContent = () => {
    if (activeOption === 'Allocation' && !activeSubOption) {
        return <div>Content for Allocation</div>;
      }
      if (activeSubOption === 'Allocation panel') {
    ...
    if (activeSubOption === 'LTPL Records Option') {
      return <TableComponent />;
    }
    ...
    if (activeSubOption === 'BL Leads Option') {
      return <TableComponent />;
    }
    return null;
  };
